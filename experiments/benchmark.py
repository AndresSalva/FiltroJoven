...
def plot_convergence_all_normalized(history_df: pd.DataFrame, output_dir: Path, norm_stats: Mapping[str, Mapping[str, float]], mode: str = "cdf") -> Optional[Path]:
    ...
    title = "Convergence (CDF 0-1) - All Fitness"
    if mode == "z":
        title = "Convergence (z-score) - All Fitness"
    plt.title(title)
    plt.xlabel("Generation")
    plt.ylabel("Normalized Best Fitness" + (" (0-1)" if mode != "z" else " (z)"))
    plt.legend()
    plt.tight_layout()
...
def plot_diversity_all(history_df: pd.DataFrame, output_dir: Path) -> Optional[Path]:
    ...
    plt.title("Population Diversity - All Fitness")
    plt.xlabel("Generation")
